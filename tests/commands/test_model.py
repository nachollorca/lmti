...
def test_handle_model_cancel(console, monkeypatch, tmp_path):
...
def test_handle_model_pick_index(console, monkeypatch, tmp_path):
    monkeypatch.setattr("lmti.config.CONFIG_PATH", tmp_path / "c.yaml")
...
def test_handle_model_manual(console, monkeypatch, tmp_path, fake_prompt):
    monkeypatch.setattr("lmti.config.CONFIG_PATH", tmp_path / "c.yaml")
    monkeypatch.setattr(
...
def test_handle_model_unknown_string_selection(console, monkeypatch, tmp_path):
    monkeypatch.setattr("lmti.config.CONFIG_PATH", tmp_path / "c.yaml")
...
def test_handle_model_manual_cancel(console, monkeypatch, tmp_path, fake_prompt):
    monkeypatch.setattr("lmti.config.CONFIG_PATH", tmp_path / "c.yaml")
    monkeypatch.setattr(
        "lmti.ui.prompt_selection",
        lambda *a, **k: "Add a manual model identifier",
